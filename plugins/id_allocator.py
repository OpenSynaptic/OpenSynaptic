...
class IDAllocator:
    def __init__(self, base_dir: str = None, start_id: int = 1, end_id: int = 4294967295,
                 ...
                 lease_policy: dict = None, metrics_sink=None):
        ...
        self._metrics_emit_interval = 5

        self._lease_policy = self._normalize_lease_policy(lease_policy)
        self._metrics_emit_interval = max(1, int(self._lease_policy.get('metrics_emit_interval_seconds', 5)))
    ...
    def _load(self):
        ...
        for k, v in data.get("allocated", {}).items():
            ...
            rec.setdefault('lease_expires_at', int(rec.get('last_seen', now_ts)) + int(self._lease_policy['base_lease_seconds']))
        ...
        saved_metrics = data.get('lease_metrics', {}) if isinstance(data.get('lease_metrics', {}), dict) else {}
        if saved_metrics:
    ...
    def _normalize_lease_policy(self, policy):
        ...
        base_seconds = int(raw.get('base_lease_seconds', base_days * 86400) or (base_days * 86400))
        ...
        return {
            ...
            'rate_window_seconds': max(300, int(raw.get('rate_window_seconds', 3600) or 3600)),
            'high_rate_threshold_per_hour': max(1.0, float(raw.get('high_rate_threshold_per_hour', 60.0) or 60.0)),
            'ultra_rate_threshold_per_hour': max(1.0, float(raw.get('ultra_rate_threshold_per_hour', 180.0) or 180.0)),
            'ultra_rate_sustain_seconds': max(60, int(raw.get('ultra_rate_sustain_seconds', 600) or 600)),
            'high_rate_min_factor': min(1.0, max(0.0, float(raw.get('high_rate_min_factor', 0.2) or 0.2))),
            'adaptive_enabled': bool(raw.get('adaptive_enabled', True)),
            'ultra_force_release': bool(raw.get('ultra_force_release', True)),
            'metrics_emit_interval_seconds': max(1, int(raw.get('metrics_emit_interval_seconds', 5) or 5)),
        }
    ...
    def _stable_device_key(self, meta):
        ...
        device_meta = m.get('device_meta', {}) if isinstance(m.get('device_meta', {}), dict) else {}
    ...
    def _update_rate_metrics_nolock(self, now_ts):
        ...
        if window <= 0:
            ...
            rate = (len(self._new_device_events) * 3600.0) / float(window)
    ...
    def _effective_lease_seconds_nolock(self, now_ts):
        ...
        if rate >= ultra_th and self._ultra_rate_since > 0 and (now_ts - self._ultra_rate_since) >= sustain:
    ...
    def _reclaim_expired_nolock(self, now_ts=None):
        ...
        if reclaimed:
            ...
            self._metrics['total_reclaimed'] = int(self._metrics.get('total_reclaimed', 0) or 0) + len(reclaimed)
        return reclaimed
        ...
        with self._lock:
            if id_val in self._allocated:
                ...
                if immediate:
                    ...
                    rec['lease_expires_at'] = now_ts + lease_seconds if lease_seconds > 0 else now_ts
                rec['state'] = 'offline'
