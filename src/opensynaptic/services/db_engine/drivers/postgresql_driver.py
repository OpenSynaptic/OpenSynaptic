...
class PostgresDriver(BaseDBDriver):
    ...
    def connect(self):
        ...
        params = {'host': self.config.get('host', '127.0.0.1'), 'port': int(self.config.get('port', 5432)), 'user': self.config.get('user'), 'password': self.config.get('password'), 'dbname': self.config.get('database')}
