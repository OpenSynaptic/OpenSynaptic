...
- Check `Config.json` lease_policy values
...
### Troubleshooting:
- **IDs running out?** → Lower high_rate_threshold (docs/ID_LEASE_CONFIG_REFERENCE.md, Priority 1)
- **IDs reused too fast?** → Increase base_lease_seconds (Priority 1)
- **Metrics not showing?** → Implement metrics_sink callback (ID_LEASE_SYSTEM.md, page X)
